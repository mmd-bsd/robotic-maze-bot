...
void robot_move_to_node(Robot* robot, int target_node) {
    ...
    if (bfs_shortest_path(robot->graph, robot->current_node, target_node, path, &path_length, 
        ...
        printf("Error: Cannot find path from %d to %d\n", robot->current_node, target_node);
    }
}
...
void robot_move(Robot* robot) {
    ...
    if (robot->current_node == robot->target_node && !robot->target_found_once) {
        printf("--- Target Found! Continuing to explore... ---\n");
    ...
    if (frontier_count == 0) {
        ...
        if (robot->current_node == robot->start_node) {
            printf("Exploration complete! Returned to start.\n");
    ...
    if (nearest_frontier != -1) {
        ...
        printf("Error: No reachable frontiers.\n");
        robot->exploration_complete = true;
    }
}
...
void print_maze_state(Graph* g, Robot* robot, int* optimal_path, int optimal_path_length) {
    ...
    if (optimal_path != NULL && optimal_path_length > 0) {
        ...
        for (int i = 0; i < optimal_path_length; i++) {
            ...
            if (i < optimal_path_length - 1) printf("-> ");
        }
        printf("\n");
    }
    printf("==================\n");
...
int main() {
    ...
    for (int i = 0; i < max_steps; i++) {
        ...
        if (robot.exploration_complete) {
            printf("Simulation finished in %d steps.\n", i);
            
            printf("Calculating shortest path...\n");
            shortest_path_nodes = (int*)malloc(MAX_NODES * sizeof(int));
            if (bfs_shortest_path(&g, START_NODE, TARGET_NODE, shortest_path_nodes, 
                                 &shortest_path_length, &robot.visited_nodes, &robot.visited_edges)) {
    ...
    if (!robot.exploration_complete) {
        printf("Simulation stopped after %d steps. Map not fully explored.\n", max_steps);
    }
    
    printf("\nSimulation complete. Displaying final path...\n");
    print_maze_state(&g, &robot, shortest_path_nodes, shortest_path_length);
