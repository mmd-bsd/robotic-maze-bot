...
int calculate_distance(Point a, Point b) {
    int dx = b.x - a.x;
    int dy = b.y - a.y;
    // Return squared distance (avoiding sqrt for integer math, still works for comparison)
    return dx * dx + dy * dy;
}

// Dijkstra's algorithm to find shortest path by DISTANCE (not number of steps)
bool dijkstra_shortest_path(Graph* g, int start, int target, int* path, int* path_length) {
    int dist[MAX_NODES];
    int parent[MAX_NODES];
    bool visited[MAX_NODES];
    ...
    // Simple Dijkstra implementation using linear search for minimum
    for (int count = 0; count < g->num_nodes; count++) {
        ...
        if (u == -1 || u == target) {
            break;  // All reachable nodes processed or target reached
        }
        
        visited[u] = true;
        
        // Update distances to neighbors
        for (int i = 0; i < g->adj[u].num_neighbors; i++) {
            int v = g->adj[u].neighbors[i];
            int edge_weight = calculate_distance(g->positions[u], g->positions[v]);
            
            if (!visited[v] && dist[u] != INT_MAX && dist[u] + edge_weight < dist[v]) {
...
int main() {
    ...
    for (int i = 0; i < max_steps; i++) {
        ...
        if (robot.exploration_complete) {
            ...
            // Use Dijkstra's algorithm for shortest path by DISTANCE (not just number of steps)
            if (dijkstra_shortest_path(&g, START_NODE, TARGET_NODE, shortest_path_nodes, 
