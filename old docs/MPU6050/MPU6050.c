...
uint8_t read_i2c(unsigned char BusAddres, unsigned char Reg, unsigned char Ack)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c1, BusAddres, data, 1, 100000);
    HAL_I2C_Master_Receive(&hi2c1, BusAddres, &Data, 1, 100000);
...
void write_i2c(unsigned char BusAddres, unsigned char Reg, unsigned char Data)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c1, BusAddres, data, 2, 100000);
}
...
void MPU6050_Init()
{
    ...
    write_i2c(MPU6050_ADDRESS, RA_GYRO_CONFIG, GFS_SEL); ////Set the register GYRO_CONFIG bits

    write_i2c(MPU6050_ADDRESS, RA_ACCEL_CONFIG, 0x10); ////Set the register ACCEL_CONFIG bits as 00010000 (+/- 8g full scale range).

    write_i2c(MPU6050_ADDRESS, RA_CONFIG, EXT_SYNC_SET | DLPF_CFG ); ////Set the register CONFIG bits as 00000000 (Set Digital Low Pass Filter to ~260Hz).
