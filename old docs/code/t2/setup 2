...
/////////	Analog
uint16_t							adcv[4],IR_ReadCounter=0,IR_ADC[18],IR_Logic[18],IR_max[18],IR_min[18],IR_mid[18];
int s[18];
...
//		HAL_Delay(500);
		...
		if(loop_start)
		{
			...
			if(head==0)                       ///////// front is front
			{
				...
				if (roatating==0)
					{
								if(s[2]+s[3]+s[4]+s[5]+s[6]+s[7]>=6)
								{
									cross=1;                                     //////////////cross with both side open
								}
								else if (s[2] && s[7]==0 && (s[4] || s[5]))
								{
									cross =2;                                  /////////////////cross with left open only
									
								}
								else if (s[2]==0 && s[7] && (s[4] || s[5]))
								{
									cross =3;                                  /////////////////////////////cross with right open only
									
								}
								else if (s[3]==0 && s[4]==0 && s[5]==0 && s[6]==0)
								{
		...
		//	BLT_SendData(sprintf(BLT_TX_Buffer,"cross=%d  ,step=%d,head=%d,roat=%d, %.2f,%.2f\r\n",cross,step,head , roatating,Z_Angle,Z_AngleSetPoint));
		}
